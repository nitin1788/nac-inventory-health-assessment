 ...
 * form-action 'self') silently blocked the redirect page's inline
 * onload auto-submit AND any cross-origin form submission, so the
 * browser could never reach PayU's checkout — see payment.payu.controller.ts
 * and payment.provider.payu.ts.
 *
