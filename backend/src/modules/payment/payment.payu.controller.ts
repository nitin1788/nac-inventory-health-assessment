 ...
 * page. Every failure path (bad/unknown/non-PayU order, or an
 * unexpected error building the form) redirects the customer to the
 * existing payment-retry page instead, exactly like
 * payuSuccessCallbackController's own failure paths below — no report
 * is ever unlocked or delivered from this route; it only ever sends the
 * browser onward, to PayU or back to retry.
 */
